...
def main():
    ...
    # Step 2: Define competitor URLs
    competitor_urls = [
        'https://www.gov.uk/uk-family-visa/partner-spouse',
        'https://www.gov.uk/uk-family-visa',
        'https://immigrationbarrister.co.uk/personal-immigration/partners-family-visas/spouse-visa/',
        'https://immigrationlawyers-london.com/uk-family-visas/spouse-visa.php'
    ]

    # Step 3: Scrape competitor contents and collect site names
    competitor_texts, site_names = scrape_competitors(competitor_urls)
    if not competitor_texts:
        print("No content scraped from any URL. Exiting.")
        return
    else:
        print(f"\nNumber of competitor contents scraped: {len(competitor_texts)}")

    # Step 4: Preprocess each competitor's text
    lemmatized_texts = [preprocess_text(text) for text in competitor_texts]

    # Step 5: Extract keywords using TF-IDF
    tfidf_keywords, vectorizer = extract_keywords_tfidf(lemmatized_texts, max_features=5000)

    # Step 6: Compute total counts for all potential keywords
    ...
    # Step 7: Rank terms within their n-gram groups
    ngram_groups = defaultdict(list)
    for idx, (term, score) in enumerate(tfidf_keywords):
        num_words = len(term.split())
        ngram_groups[num_words].append((term.lower(), score))

    # Apply ranking within n-gram groups
    ...
    # Step 8: Generate site name sequential phrases
    site_name_phrases = set()
    for site_name in site_names:
        words = site_name.split()
        for i in range(len(words) - 1):
            phrase = ' '.join(words[i:i+2])
            site_name_phrases.add(phrase.lower())

    # Step 9: Define Seed Keyword
    seed_keyword = 'spouse visa uk'

    # Step 10: Generate seed keyword variations
    seed_variations = generate_seed_variations(seed_keyword)
    logging.info(f"Seed Variations: {seed_variations}")

    # Step 11: Define inclusion limits based on n-gram length
    ngram_inclusion_limits = {
        1: 50,    # Limit for single-word terms
        2: 150,   # Limit for two-word phrases
        3: 150,   # Limit for three-word phrases
        4: 150,   # Limit for four-word phrases
    }
    ngram_inclusion_limits_default = 50  # For phrases with more than 4 words

    # Step 12: Filter keywords based on relevancy and criteria
    filtered_keywords = filter_keywords(
        ...
        ngram_inclusion_limits=ngram_inclusion_limits,
        ngram_inclusion_limits_default=ngram_inclusion_limits_default
    )

    if not filtered_keywords:
        print("No keywords passed the filtering criteria.")
        return
    else:
        logging.info(f"Number of keywords after filtering: {len(filtered_keywords)}")

    # Step 13: Ensure seed keyword and its variations are included
    for variation in seed_variations:
        if variation not in [term.lower() for term, _ in filtered_keywords]:
            ...
            if score > 0:
                ...
                logging.info(f"Added missing seed variation: '{variation}' with score {score}")

    # Step 14: Limit multi-word phrases (3 or more words) to top N
    multi_word_phrases = [(term, score) for term, score in filtered_keywords if len(term.split()) >= 3]
    multi_word_phrases_sorted = sorted(multi_word_phrases, key=lambda x: -x[1])[:150]

    # Combine all filtered keywords
    filtered_keywords = [item for item in filtered_keywords if len(item[0].split()) < 3]
    filtered_keywords.extend(multi_word_phrases_sorted)

    # Step 15: Deduplicate terms with adjusted logic
    final_keywords = deduplicate_terms(
        filtered_keywords,
        seed_variations=seed_variations,
    ...
    if not final_keywords:
        print("No keywords remained after deduplication.")
        return
    else:
        logging.info(f"Number of keywords after deduplication: {len(final_keywords)}")

    # Step 16: Prepare counts with average and range
    keyword_counts_display = {}
    for term, _ in final_keywords:
        counts = counts_info.get(term.lower(), {'avg': 0, 'range': 0})
        avg_count = int(counts['avg'])
        range_count = int(counts['range'])
        # Create a list of the two numbers in ascending order
        numbers = sorted([avg_count, range_count])
        keyword_counts_display[term] = f"{numbers[0]}-{numbers[1]}"

    # Step 17: Arrange the results as per your request
    # Separate seed phrase, variations, and other keywords
    seed_phrase = seed_keyword
    variations = [term for term, score in final_keywords if term.lower() in seed_variations and term.lower() != seed_keyword.lower()]
    other_keywords = [(term, score) for term, score in final_keywords if term.lower() not in seed_variations and term.lower() != seed_keyword.lower()]
    # Sort other keywords by TF-IDF score (highest to smallest)
    other_keywords_sorted = sorted(other_keywords, key=lambda x: -x[1])

    # Step 18: Display the results
    print("\nFiltered Top Keywords and Phrases with TF-IDF Score and Counts:")
    print("-" * 80)
    # Display Seed Phrase
    print(f"Seed Phrase:")
    counts_display = keyword_counts_display.get(seed_phrase, '0-0')
    score = next((s for t, s in final_keywords if t.lower() == seed_phrase.lower()), 0)
    print(f"{seed_phrase:<50} {score:<15.4f} {counts_display:<15}")
    print("-" * 80)
    # Display Variations
    if variations:
        print("Variations of Seed Phrase:")
        for term in variations:
            counts_display = keyword_counts_display.get(term, '0-0')
            score = next((s for t, s in final_keywords if t == term), 0)
            print(f"{term:<50} {score:<15.4f} {counts_display:<15}")
        print("-" * 80)
    else:
        print("No variations of the seed phrase found.")
        print("-" * 80)
    # Display Other Keywords
    print("Other Keywords (sorted by TF-IDF score):")
    for term, score in other_keywords_sorted:
        counts_display = keyword_counts_display.get(term, '0-0')
        print(f"{term:<50} {score:<15.4f} {counts_display:<15}")
    print("-" * 80)
